...
namespace StarWars_Mario_Klimkova
{
    public partial class ThirdLevel : Form
    {
        ...
        private void GameResult()
        {
            foreach (Control enemy in this.Controls)
            {
                if (enemy is PictureBox && (string)enemy.Tag == "enemy")
                {
                    ...
                    {
                        if (coin is PictureBox && (string)coin.Tag == "coin")
                        {
                            ...
                            if (player.Bounds.IntersectsWith(enemy.Bounds))
                            {
                                ...
                                right = false;
                                jump = false;
                                player.Location = new Point(448, 149);
                                life += 1;
                                LifeStatus();
                            }

                            if (enemy.Bounds.IntersectsWith(coin.Bounds))
                            {
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void ThirdLevel_KeyDown(object sender, KeyEventArgs e)
        {
            switch (e.KeyCode)
            {
                case Keys.Right:
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void ThirdLevel_KeyUp(object sender, KeyEventArgs e)
        {
            switch (e.KeyCode)
            {
                case Keys.Right:
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void ThirdLevel_FormClosed(object sender, FormClosedEventArgs e)
        {
            Application.Exit();
        }
